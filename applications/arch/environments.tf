locals {
  database_url  = "postgresql://${local.app_name}:${module.this_db.password}@${data.terraform_remote_state.stack.outputs.db_address}:${data.terraform_remote_state.stack.outputs.db_port}/${local.app_name}?pool=50"
  mount_volumes = "/var/app/arch-derivatives=${module.this_derivative_volume.dns_name}:/var/app/arch-working=${module.this_working_volume.dns_name}"
}
