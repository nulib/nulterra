...
resource "aws_sqs_queue" "this_ui_fifo_deadletter_queue" {
  name                        = "${data.terraform_remote_state.stack.outputs.stack_name}-donut-ui-dead-letter-queue.fifo"
  fifo_queue                  = true
...
resource "aws_sqs_queue" "this_ui_fifo_queue" {
  name                        = "${data.terraform_remote_state.stack.outputs.stack_name}-donut-ui-queue.fifo"
  fifo_queue                  = true
  content_based_deduplication = false
  delay_seconds               = 0
...
resource "aws_sqs_queue" "this_batch_fifo_deadletter_queue" {
  name                        = "${data.terraform_remote_state.stack.outputs.stack_name}-donut-batch-dead-letter-queue.fifo"
  fifo_queue                  = true
  content_based_deduplication = false
  tags                        = local.common_tags
}
...
resource "aws_sqs_queue" "this_batch_fifo_queue" {
  name                        = "${data.terraform_remote_state.stack.outputs.stack_name}-donut-batch-queue.fifo"
  fifo_queue                  = true
  content_based_deduplication = false
  delay_seconds               = 0
  visibility_timeout_seconds  = 3600
...
data "aws_iam_policy_document" "this_bucket_access" {
  ...
  statement {
    ...
    resources = [data.terraform_remote_state.stack.outputs.elasticsearch_arn]
  }
...
data "null_data_source" "ssm_parameters" {
  inputs = {
    ...
    "aws/buckets/manifests"    = data.terraform_remote_state.stack.outputs.iiif_pyramid_bucket
    "aws/buckets/pyramids"     = data.terraform_remote_state.stack.outputs.iiif_pyramid_bucket
    "aws/buckets/uploads"      = aws_s3_bucket.this_uploads.id
    "aws/lambdas/pyramid"      = module.this_pyramid_trigger.function_arn
    "common_indexer/endpoint"  = data.terraform_remote_state.stack.outputs.elasticsearch_endpoint
    "domain/host"              = local.domain_host
    "geonames_username"        = "nul_rdc"
    "iiif/endpoint"            = data.terraform_remote_state.stack.outputs.iiif_endpoint
    "metadata/endpoint"        = data.terraform_remote_state.stack.outputs.metadata_endpoint
    "solr/url"                 = "${data.terraform_remote_state.stack.outputs.index_endpoint}donut"
