...
resource "aws_sqs_queue" "this_ui_deadletter_queue" {
  name       = "${data.terraform_remote_state.stack.outputs.stack_name}-avr-ui-dead-letter-queue"
  fifo_queue = false
  tags       = local.common_tags
}

resource "aws_sqs_queue" "this_ui_queue" {
  name                       = "${data.terraform_remote_state.stack.outputs.stack_name}-avr-ui-queue"
  fifo_queue                 = false
  delay_seconds              = 0
...
resource "aws_sqs_queue" "this_batch_deadletter_queue" {
  name       = "${data.terraform_remote_state.stack.outputs.stack_name}-avr-batch-dead-letter-queue"
  fifo_queue = false
  tags       = local.common_tags
}

resource "aws_sqs_queue" "this_batch_queue" {
  name                       = "${data.terraform_remote_state.stack.outputs.stack_name}-avr-batch-queue"
  fifo_queue                 = false
  delay_seconds              = 0
  visibility_timeout_seconds = 3600
...
data "null_data_source" "ssm_parameters" {
  inputs = {
    ...
    "email/notification" = var.email_notification
    "email/support"      = var.email_support
    ...
    "solr/url"           = "${data.terraform_remote_state.stack.outputs.index_endpoint}avr"
    "streaming/http_base" = "https://${coalesce(
      var.streaming_hostname,
      aws_route53_record.this_cloudfront.fqdn,
    )}/"
