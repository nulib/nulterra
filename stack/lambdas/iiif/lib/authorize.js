...
async function authorize(token, id, referer) {
  ...
  var docUrl = url.parse(url.resolve(elasticSearch, `_all/_search`));
  var request = await makeRequest("POST", docUrl, JSON.stringify(query));
  ...
  if (
    isObject(doc.hits) &&
    doc.hits.total >= 1 &&
    isString(doc.hits.hits[0]._source.visibility)
  ) {
    switch (doc.hits.hits[0]._source.visibility.toLowerCase()) {
      case "open":
        return true;
      case "authenticated":
