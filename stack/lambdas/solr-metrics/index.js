...
async function collectMetrics(solrUrl, region) {
  ...
  for (var collectionName in cluster.cluster.collections) {
    ...
    metrics.add('DocumentCount', docCount, { SolrCollection: collectionName });
    ...
    while (facets.length > 0) { 
      ...
      if (! modelName.match(/^(Hydra|ActiveFedora)::/)) {
        metrics.add('DocumentCount', modelCount, { SolrCollection: collectionName, Model: modelName });
      }
    }
    ...
    for (var shardName in collectionInfo.shards) {
      ...
      metrics.add('ActiveReplicaCount', replicaCount, { SolrCollection: collectionName, Shard: shardName });
    }
  }
