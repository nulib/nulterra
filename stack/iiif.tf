...
resource "aws_api_gateway_base_path_mapping" "iiif_domain_mapping" {
  ...
  domain_name = aws_api_gateway_domain_name.iiif_domain_name[0].domain_name
}
...
resource "aws_route53_record" "iiif" {
  ...
  alias {
    name                   = aws_api_gateway_domain_name.iiif_domain_name[0].regional_domain_name
    zone_id                = aws_api_gateway_domain_name.iiif_domain_name[0].regional_zone_id
...
locals {
  ...
  fudged_record_list = concat(aws_route53_record.iiif.*.name, ["DUMMY_ITEM"])
