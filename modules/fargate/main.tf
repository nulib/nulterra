...
resource "aws_lb" "this_load_balancer" {
  ...
  subnets = split(
    ",",
    var.internal == "true" ? join(",", var.private_subnets) : join(",", var.public_subnets),
  )
...
resource "aws_lb_listener" "this_listener" {
  count             = var.load_balanced == "true" ? 1 : 0
  load_balancer_arn = aws_lb.this_load_balancer[0].arn
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_ecs_service" "this_service" {
  ...
  network_configuration {
    ...
    security_groups = [concat(
      var.security_groups,
      aws_security_group.this_instance_security_group.*.id,
    )]
